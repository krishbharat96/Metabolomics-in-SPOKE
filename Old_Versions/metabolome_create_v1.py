...
def attempt_create_react_pathway(reactome_id):
    ...
    if analyze_json(path_events):
        for item in path_events['hasEvent']:
            ...
            event_info = requests.get(get_reactome_id_information.format(reactome_id=react_id)).json()
...
def create_reactome_pathway(reactome_id):
    fin_dict = {reactome_id:{}}
    try:
        path_events = requests.get(get_pathway_ancestors.format(reactome_id=reactome_id)).json()
        if analyze_json(path_events):
